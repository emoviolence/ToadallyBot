...
bot.on('message', function(user, userID, channelID, message, event) {
	...
	if (lowMessage.indexOf(pre + "testthatemb") === 0) {
		bot.sendMessage({
			to: channelID,
			embed: {
			title: 'some title',
...
function help(user, userID, channelID, message, event) {
	bot.sendMessage({
		to: channelID,
		message: "<:barron2:380911515039170562> Ribbit! Ribbit! I am Toadally Awesome, the ultimate Yu-Gi-Oh! Bot! <:god:374019990308913152>"
	});
}
...
function commands(user, userID, channelID, message, event) {
	...
	out += "See the readme for details and other commands some random dude skimmed over: <https://github.com/AlphaKretin/bastion-bot/>";
	bot.sendMessage({
		to: userID,
		message: out
	});
	...
	return new Promise(function(resolve, reject) {
		...
		request('https://yugiohprices.com/api/get_card_prices/' + name[1], function(error, response, body) {
			...
			if (types.indexOf("Monster") > -1) {
				let typesStr = types.toString().replace("Monster", AddEmote(getRace(index, outLang),emotemode)).replace(/,/g, "/");
				...
				if (lv > 0) { //is trap monster
					let typesStr = AddEmote(getRace(index, outLang),emotemode) + "/" + types.toString().replace(/,/g, "/");
					out += "**Type**: " + typesStr + " **Attribute**: " + AddEmote(getAtt(index, outLang),emotemode) + "\n";
	...
	if (index > -1 && index in ids.en) {
		...
		switch (prop) {
			...
			case "type":
				...
				if (types.indexOf("Monster") > -1) {
					out = types.toString().replace("Monster", AddEmote(getRace(index, "en"),emotemode)).replace(/,/g, "/");
				} else {
					let lv = getLevelScales(index, "en")[0];
					if (lv > 0) { //is trap monster
						out = AddEmote(getRace(index, "en"),emotemode) + "/" + types.toString().replace(/,/g, "/");
					} else {
...
function AddEmote(args,emotemode) {
